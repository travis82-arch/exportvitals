...
import { loadFromLocalCache, importZip, getAvailableDates, getDay, getRange, getBaseline, getStoreSnapshot, setUiSnapshot } from './store/dataStore.js';
...
const fmt = (n, unit = '') => (n == null ? '<span class="placeholder">Not in this export</span>' : `${Number(n).toFixed(1)}${unit}`);
const emoji = (v) => (v ? '✓' : '✗');
...
function currentSnapshot() {
  ...
  const range = start && end ? getRange(start, end) : { dailySleep: [], dailyReadiness: [], dailyActivity: [], dailySpo2: [], derivedNightlyVitals: [] };
...
if (page === 'index') {
  ...
  const contributors = day?.dailySleep?.contributors ? Object.entries(day.dailySleep.contributors).map(([k, v]) => `<li>${k}: ${v}</li>`).join('') : '<li class="placeholder">Not in this export</li>';
  ...
  <div class="kpi"><div class="kpi-label">Breathing regularity (BDI)</div><div class="kpi-value">${fmt(day?.dailySpo2?.breathingDisturbanceIndex)}</div></div>
  </div><h3>Contributors</h3><ul>${contributors}</ul><h3>Overnight HR trend</h3>${simpleTrendSvg((getRange(selectedDate, selectedDate).derivedNightlyVitals), 'rhr_night_bpm')}<p class="small muted">Sleep duration/stages are not present in these CSV exports.</p></section>`;
} else if (page === 'readiness') {
  const contributors = day?.dailyReadiness?.contributors ? Object.entries(day.dailyReadiness.contributors).map(([k, v]) => `<li>${k}: ${v}</li>`).join('') : '<li class="placeholder">Not in this export</li>';
  const baseTemp = getBaseline('temperatureDeviation', settings.baselineWindow, selectedDate);
  app.innerHTML = `<section class="card"><h2>Readiness</h2>${renderDateStrip()}<div class="kpi"><div class="kpi-label">Readiness score</div><div class="kpi-value">${fmt(day?.dailyReadiness?.score)}</div></div><div class="kpi top-gap"><div class="kpi-label">Temp deviation</div><div class="kpi-value">${fmt(day?.dailyReadiness?.temperatureDeviation, ' °C')}</div><div class="small muted">Baseline ${fmt(baseTemp, ' °C')}</div></div><h3>Contributors</h3><ul>${contributors}</ul></section>`;
} else if (page === 'activity') {
  const range = getRange(getAvailableDates().at(-14) || selectedDate, selectedDate);
  const stepsAvg = range.dailyActivity.length ? range.dailyActivity.reduce((s, r) => s + (r.steps || 0), 0) / range.dailyActivity.length : null;
  const calsAvg = range.dailyActivity.length ? range.dailyActivity.reduce((s, r) => s + (r.activeCalories || 0), 0) / range.dailyActivity.length : null;
  app.innerHTML = `<section class="card"><h2>Activity</h2>${renderDateStrip()}<div class="grid vitals-grid"><div class="kpi"><div class="kpi-label">Activity score</div><div class="kpi-value">${fmt(day?.dailyActivity?.score)}</div></div><div class="kpi"><div class="kpi-label">Steps</div><div class="kpi-value">${fmt(day?.dailyActivity?.steps)}</div></div><div class="kpi"><div class="kpi-label">Active calories</div><div class="kpi-value">${fmt(day?.dailyActivity?.activeCalories)}</div></div><div class="kpi"><div class="kpi-label">14d avg steps</div><div class="kpi-value">${fmt(stepsAvg)}</div></div><div class="kpi"><div class="kpi-label">14d avg cals</div><div class="kpi-value">${fmt(calsAvg)}</div></div></div></section>`;
} else if (page === 'vitals') {
  ...
  app.innerHTML = `<section class="card"><h2>Vitals</h2>${renderDateStrip()}<div class="grid vitals-grid"><div class="kpi"><div class="kpi-label">RHR Night</div><div class="kpi-value">${fmt(day?.derivedNightlyVitals?.rhr_night_bpm, ' bpm')}</div><div class="small muted">Baseline ${fmt(baseRhr)} · Δ ${fmt((day?.derivedNightlyVitals?.rhr_night_bpm ?? null) - (baseRhr ?? 0))}</div></div><div class="kpi"><div class="kpi-label">Estimated HRV</div><div class="kpi-value">${fmt(day?.derivedNightlyVitals?.hrv_rmssd_proxy_ms, ' ms')}</div><div class="small muted">Baseline ${fmt(baseHrv)} · Δ ${fmt((day?.derivedNightlyVitals?.hrv_rmssd_proxy_ms ?? null) - (baseHrv ?? 0))}</div></div><div class="kpi"><div class="kpi-label">SpO2</div><div class="kpi-value">${fmt(day?.dailySpo2?.spo2Average, '%')}</div><div class="small muted">Baseline ${fmt(baseSpo2, '%')}</div></div><div class="kpi"><div class="kpi-label">Temp deviation</div><div class="kpi-value">${fmt(day?.dailyReadiness?.temperatureDeviation, ' °C')}</div></div></div></section>`;
} else if (page === 'trends') {
  ...
  document.getElementById('zip').addEventListener('change', async (event) => {
    ...
    if (!file) return;
    const report = await importZip(file, settings);
  ...
  const metadata = { exportedAt: new Date().toISOString(), digest: shortHash(JSON.stringify(uiData)), selectedDate, baselineWindow: settings.baselineWindow };
  ...
  document.getElementById('jsonExport').addEventListener('click', () => download('normalized_all.json', JSON.stringify({ metadata, uiData }, null, 2)));
  document.getElementById('csvExport').addEventListener('click', () => download('derived_nightly_vitals.csv', toCsv(uiData.range.derivedNightlyVitals)));
  ...
  const coverageText = ['dailySleep', 'dailyReadiness', 'dailyActivity', 'dailySpo2', 'heartRate', 'sleepTime'].map((k) => `${k}:${emoji(coverage[k])}`).join(' · ');
  app.innerHTML = `<section class="card"><h2>Settings</h2><p class="small muted">Data coverage: ${coverageText || 'Import data to compute coverage'}</p><label>Baseline window <input id="baseline" type="number" min="7" value="${settings.baselineWindow}"/></label><label><input id="developer" type="checkbox" ${settings.developerMode ? 'checked' : ''}/> Developer mode</label><label><input id="remember" type="checkbox" ${settings.rememberDerived ? 'checked' : ''}/> Remember last import</label><label>Night window mode <select id="nightWindowMode"><option value="auto">auto</option><option value="sleep-time">sleep-time</option><option value="settings">settings fallback only</option></select></label><label>Fallback start <input id="fallbackStart" type="time" value="${settings.fallbackStart}" /></label><label>Fallback end <input id="fallbackEnd" type="time" value="${settings.fallbackEnd}" /></label></section>`;
  ...
  if (!settings.developerMode) {
    ...
    app.innerHTML = `<section class="card"><h2>Debug</h2><pre class="status">${JSON.stringify(debugInfo, null, 2)}</pre></section>`;
  }
